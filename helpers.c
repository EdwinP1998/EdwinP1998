...
void PrintMenuOptions(void)
{
	...
	printf("3. Modify Ranged Attack\n");
	printf("4. Modify Ranged Damage\n");
	printf("5. Modify Melee Damage\n");
}
...
int ModifyDefense(HANDLE hTarget, void* playerStatsPointer)
{
	...
	// Get the player's current defense
	...
	{
		printf("Your defense has been changed.");
	}

	return TERROR_SUCCESS;
}
int ModifyRangedAttack(HANDLE hTarget, void* playerStatsPointer)
{
	void* ptr;
	unsigned int desiredAttack;
	...
	// ptr now contains the address to the player stats structure. 
	// We'll add 0x40 to this address to point to the player's ranged attack.
	void* addr = (void*)((int)ptr + 0x38);

	// Get the player's current ranged attack
	unsigned int currentAttack;
	if (!ReadProcessMemory(hTarget, addr, &currentAttack, sizeof(int), NULL))
	{
		printf("ReadProcessMemory() failed with error: %d.\n", GetLastError());
		return TERROR_MODIFY_HEALTH_FAILED;
	}

	// Ask the user what they want their ranged attack to be (unsigned int max)
	printf("Your base attack is %d.\nWhat would you like your new base attack to be? >> ", currentAttack);
	scanf_s("%u", &desiredAttack);

	// INT_MAX = 2147483647
	if (desiredAttack > INT_MAX || desiredAttack <= 0)
	{
		printf("ERROR: I can only modify attack within the range [1,%d]\nHealth unchanged.\n", INT_MAX);
		return TERROR_MODIFY_HEALTH_FAILED;
	}

	if (!WriteProcessMemory(hTarget, addr, &desiredAttack, sizeof(desiredAttack), NULL))
	{
		printf("WriteProcessMemory() failed with error: %d.\n", GetLastError());
		return TERROR_MODIFY_HEALTH_FAILED;
	}
	...
	{
		printf("Your attack has been changed.");
	}

	return TERROR_SUCCESS;
}
int ModifyRangedDamage(HANDLE hTarget, void* playerStatsPointer)
{
	void* ptr;
	unsigned int desiredDamage;
	...
	// ptr now contains the address to the player stats structure. 
	// We'll add 0x40 to this address to point to the player's ranged damage.
	void* addr = (void*)((int)ptr + 0x48);

	// Get the player's current ranged damage
	unsigned int currentAttack;
	if (!ReadProcessMemory(hTarget, addr, &currentAttack, sizeof(int), NULL))
	{
		printf("ReadProcessMemory() failed with error: %d.\n", GetLastError());
	...
	// Ask the user what they want their ranged damage to be (unsigned int max)
	printf("Your base damage is %d.\nWhat would you like your new base damage to be? >> ", currentAttack);
	scanf_s("%u", &desiredDamage);

	// INT_MAX = 2147483647
	if (desiredDamage > INT_MAX || desiredDamage <= 0)
	{
		printf("ERROR: I can only modify damage within the range [1,%d]\nHealth unchanged.\n", INT_MAX);
		return TERROR_MODIFY_HEALTH_FAILED;
	}

	if (!WriteProcessMemory(hTarget, addr, &desiredDamage, sizeof(desiredDamage), NULL))
	{
		printf("WriteProcessMemory() failed with error: %d.\n", GetLastError());
		return TERROR_MODIFY_HEALTH_FAILED;
	}
	...
	{
		printf("Your damage has been changed.");
	}

	return TERROR_SUCCESS;
}
int ModifyMeleeDamage(HANDLE hTarget, void* playerStatsPointer)
{
	void* ptr;
	unsigned int desiredDamage;
	...
	// ptr now contains the address to the player stats structure. 
	// We'll add 0x40 to this address to point to the player's melee damage.
	void* addr = (void*)((int)ptr + 0x48);

	// Get the player's current melee damage
	unsigned int currentAttack;
	if (!ReadProcessMemory(hTarget, addr, &currentAttack, sizeof(int), NULL))
	{
		printf("ReadProcessMemory() failed with error: %d.\n", GetLastError());
	...
	// Ask the user what they want their melee damage to be (unsigned int max)
	printf("Your base damage is %d.\nWhat would you like your new base damage to be? >> ", currentAttack);
	scanf_s("%u", &desiredDamage);
